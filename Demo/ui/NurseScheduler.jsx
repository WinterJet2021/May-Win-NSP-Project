...
function ScheduleTable({ data }) {
  if (!data.dates.length || !data.nurses.length) return null;
    ...
    <div className="overflow-x-auto">
      ...
      <table className="min-w-full bg-white border border-gray-300">
        ...
        <tbody>
          ...
          {/* Coverage Requirements and Shortfall Row */}
...
function CoverageChart({ data }) {
  ...
  useEffect(() => {
    ...
    // Prepare chart data
    const chartData = {
      labels: data.dates.map(date => new Date(date).toLocaleDateString('en-US', {
        month: 'short', day: 'numeric'
      })),
      datasets: [
        {
          label: 'Day Shift',
          data: data.dates.map(date => shiftCounts[date]['D']),
        ...
        {
          label: 'Evening Shift',
          data: data.dates.map(date => shiftCounts[date]['E']),
        ...
        {
          label: 'Night Shift',
          data: data.dates.map(date => shiftCounts[date]['N']),
        ...
        {
          label: 'Day Required',
          data: data.dates.map(date => requirements[date]?.['D'] || 0),
          type: 'line',
          borderColor: '#1565c0',
          borderWidth: 2,
          fill: false,
        ...
        {
          label: 'Evening Required',
          data: data.dates.map(date => requirements[date]?.['E'] || 0),
          type: 'line',
          borderColor: '#ef6c00',
          borderWidth: 2,
          fill: false,
        ...
        {
          label: 'Night Required',
          data: data.dates.map(date => requirements[date]?.['N'] || 0),
          type: 'line',
          borderColor: '#2e7d32',
          borderWidth: 2,
          fill: false,
    ...
    chartInstance.current = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          title: {
            display: true,
            text: 'Nurse Coverage vs. Requirements',
            font: {
              size: 16
            }
          },
  ...
  const exportCSV = (type) => {
    ...
    if (type === 'assignments') {
      csvData = Papa.unparse({
        fields: ["date", "shift", "nurse_id"],
        data: data.assignments
      });
      filename = "assignments.csv";
    } else {
      csvData = Papa.unparse({
        fields: ["date", "shift", "unmet"],
        data: data.shortfall
      });
...
function SolverControls({ onOptimize, filePath, disabled, apiBaseUrl = '' }) {
  ...
  const handleOptimize = () => {
    ...
    // Make the API call to start optimization
...
export function NurseSchedulerApp({ apiBaseUrl = '' }) {
  ...
  const handleDataLoaded = (data) => {
    ...
    // Fetch solution if available (for sample data)
    fetchSolution();
  };
  ...
  const fetchSolution = () => {
    ...
    .then(data => {
      if (data.error) {
        // No solution yet, that's okay
        return;
      }
      
      setScheduleData(data);
      setObjectiveValue(data.objective);
      setDataLoaded(true);
    })
    .catch(err => {
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <header className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Optimize nurse schedules while meeting coverage requirements and preferences
        </p>
      </header>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-red-50 p-3 rounded">
                  ...
                  <div className="text-2xl font-bold">
                    {new Set(scheduleData.shortfall.map(s => s.date)).size}
                  </div>
                </div>
              </div>
            </div>
