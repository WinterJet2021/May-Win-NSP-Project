...
def ensure_date_horizon(cfg: Dict[str, Any]) -> None:
    if "date_horizon" in cfg and cfg["date_horizon"].get("start") and cfg["date_horizon"].get("end"):
        return
    dates = [row["date"] for row in cfg.get("coverage_requirements", []) if "date" in row]
    if not dates:
...
def main():
    try:
        ...
        assignments, shortfall, obj, diag = solve_from_cfg_gurobi(
            cfg, normalize_nsp_json, time_limit_sec=60, threads=8
        )

        print(f"\n✓ Solve complete. Objective = {obj if obj is not None else 'None'}")
        print(f"• Assignments: {len(assignments)}")
        days_with_shortfall = len({(r['date'], r['shift']) for r in shortfall})
        print(f"• Days with shortfall: {days_with_shortfall}")
        print(f"• Status: {diag.get('status_name')}  SolCount: {diag.get('solcount')}\n")
